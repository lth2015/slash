...
LLM（Gemini 2.5 Flash）被用在两个只读端点，共享同一套 HARD RULES：

1. `POST /explain` —— 解读已发生的结果 / 生成诊断报告 / 解释错误
2. `POST /help` —— 自然语言回答"这个 cockpit 能做什么"。prompt 喂入全量 skill catalog；返回的 `suggested_commands` 在 server 侧过白名单，任何不在 catalog 里的"/namespace verb" 头部直接丢弃（防 LLM 幻觉新命令）；关闭时退回 deterministic catalog 列表
